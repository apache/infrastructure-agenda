...
def test_dir(data_dir):
    svndir = svn.Dir(os.path.join(data_dir, 'repos', 'foundation_board'),
                      filter=r'board_agenda_\d{4}_\d{2}_\d{2}\.txt',
                      recurse=True)
    assert svndir.path == os.path.join(data_dir, 'repos', 'foundation_board')
...
def test_dir_with_defaults(data_dir):
    svn.Dir(os.path.join(data_dir, 'repos', 'minutes'))
...
def test_file_missing(data_dir):
    svndir = svn.Dir(os.path.join(data_dir, 'repos', 'foundation_board'),
                  filter=r'board_agenda_\d{4}_\d{2}_\d{2}\.txt',
                  recurse=True)
